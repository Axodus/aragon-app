...
export const parseFeatureFlagOverridesFromCookie = (cookieSource?: string | null): FeatureFlagOverrides => {
    ...
    try {
        ...
        const overridesCookie = cookies.find((item) => item.startsWith(`${FEATURE_FLAGS_OVERRIDES_COOKIE_NAME}=`));
        ...
        for (const [key, val] of Object.entries(parsed)) {
            if (typeof val === 'boolean' && FEATURE_FLAG_DEFINITIONS.some((item) => item.key === key)) {
...
export const serializeFeatureFlagOverridesToCookie = (overrides: FeatureFlagOverrides): string => {
