...
This project uses `scripts/setupEnv.sh` to copy one of the files from `config/` into `.env.local` (and also create a
root `.env.<env>` file for reference). Use `.env.example` as a template for the variables.
...
| **Develop**    | https://dev.app.aragon.org  | Automatic on `main` branch push              | `.env.development` |
